...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	// normalization
	if(sig_grs==0&&exp!=0) return 0;
	bool overflow = false; // true if the result is INFINITY or 0 during normalize
    uint64_t sticky=0;
	if ((sig_grs >> (23 + 3)) > 1 || exp < 0)//（太大或阶码下溢）
	{
		// normalize toward right
		while ((((sig_grs >> (23 + 3)) > 1) && exp < 0xff) // condition 1（需要右规且阶码没上溢）
			   ||										   // or
			   (sig_grs > 0x04 && exp < 0)				   // condition 2(尾数非0且阶码下溢)
			   )
			       
		{        	sticky = sticky | (sig_grs & 0x1);
		         sig_grs = sig_grs >> 1;//右规
	          sig_grs |= sticky;//存粘位
              exp++;
			/* TODO: shift right, pay attention to sticky bit*/
		
		}
		if (exp >= 0xff)//最后右规上溢了， 根据符号将结果置为 +∞ 或 -∞ (全1阶码全0尾数)
		{
		    exp=0xff;
		    sig_grs=0;
		...
		if (exp < 0)//阶码下溢(乘除),根据符号将结果置为 +0 或 -0
		{ 
	...
	else if (((sig_grs >> (23 + 3)) == 0) && exp > 0)//需要左规且没溢出
	{
		// normalize toward left
		while (((sig_grs >> (23 + 3)) == 0) && exp > 0)//标准左规
		{
             sig_grs=sig_grs<<1;
             exp--;
		...
		if (exp == 0)//规到了阶码等于0，化为非规格化数（右移一位）(但是阶码不加了)
		{
   	         sticky = sticky | (sig_grs & 0x1);
		         sig_grs = sig_grs >> 1;//右规
	          sig_grs |= sticky;//存粘位
	...
	else if (exp == 0 && sig_grs >> (23 + 3) == 1)//两个非规格化数得到一个规格化数（阶码+1）
	{
  ...
  int grs=sig_grs&0x7;
	...
	if (!overflow)//没溢出,向最近的偶数舍入,由于在舍入过程中可能涉及尾数加一。因此还要再判断一次是否需要右规。对于规格化浮点数，不要忘记丢弃最高的隐藏位。
	{
   ...
   if(((sig_grs&(0x08))>>3)==1) {sig_grs+=0x08;
   while(((sig_grs>>(23+3))>1&&exp<0xff)||(sig_grs>0x04&&exp<0))
   {
   sig_grs=sig_grs>>1;
   exp++;
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	fb.val = b;
	// infity, NaN
	if (fa.exponent == 0xff)//太大
	{
		return a;
	}
	if (fb.exponent == 0xff)//太大
	{
		return b;
	}

	if (fa.exponent > fb.exponent)//对阶
	{
		fa.val = b;
		fb.val = a;
	}
 
	uint32_t sig_a, sig_b,sig_res;//转为原数
	sig_a = fa.fraction;//提取尾数
	if (fa.exponent != 0)
  ...
  shift = (fb.exponent == 0 ? fb.exponent + 1 : fb.exponent) - (fa.exponent == 0 ? fa.exponent + 1 : fa.exponent);
	...
	while (shift > 0)//对齐
	{
		sticky = sticky | (sig_a & 0x1);//只要舍入位再右规出1就永远置一
		sig_a = sig_a >> 1;//右规
		sig_a |= sticky;//存粘位
		shift--;
	}
	...
	return internal_normalize(f.sign, exp_res, sig_res);//规格化
}
