...
PDE kpdir[NR_PDE] align_to_page;				// kernel page directory,1024个4b,4kb
PTE kptable[PHY_MEM / PAGE_SIZE] align_to_page; // kernel page tables,128M/4kb个4kb表项，即32k个
//为什么开这么大？
...
void init_page(void)
{
	...
	/* fill PDEs and PTEs */
	pframe_idx = 0;
	for (pdir_idx = 0; pdir_idx < PHY_MEM / PT_SIZE; pdir_idx++)//128M/(1024*4kb(PTE的大小))，32个PDE？
	{
		pdir[pdir_idx].val = make_pde(ptable);
		pdir[pdir_idx + KOFFSET / PT_SIZE].val = make_pde(ptable);//?
		for (ptable_idx = 0; ptable_idx < NR_PTE; ptable_idx++)//1024
		{
